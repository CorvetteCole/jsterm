    ...
    const Terminal = {
        ...
        tabComplete: function (text) {
            ...
            if (parts.length === 1) {
                ...
                let pathParts = parts[0].replace(/[\/]+/, '/').split('/');
                let last = pathParts.pop();
                let dir = (pathParts.length > 0) ? this.getEntry(pathParts.join('/')) : this.cwd;
                let n;
        ...
        _addDirs: function (curDir, parentDir) {
            curDir.contents.forEach(function (entry) {
                if (entry.type === 'dir')
        ...
        _toggleBlinker: function (timeout) {
            let blinker = this.div.querySelector('#blinker'),
                stdout;
        ...
        _resetID: function (query) {
            const element = this.div.querySelector(query);
